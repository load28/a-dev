    ...
    if payload.success {
        ...
        if let Some(composite_task) = state.engine.get_composite_task(&payload.composite_task_id).await {
            ...
            if composite_task.is_completed() {
                tracing::info!(
                    "Composite task {} is fully completed! Creating final PR to main.",
                    composite_task.id
                );

                // Create final PR from parent branch to main
                let parent_branch = format!("autodev/{}", composite_task.id);
                let pr_body = format!(
                    "# {}\n\n{}\n\n## Subtasks Completed\n{}",
                    ...
                    .await
                {
                    Ok(pr) => {
                        tracing::info!(
                            "Created final PR #{} for composite task {}",
                            pr.number,
                            composite_task.id
                        );
                    }
                    Err(e) => {
                        tracing::error!(
                            "Failed to create final PR for composite task {}: {}",
                            composite_task.id,
                            e
                        );
                    }
